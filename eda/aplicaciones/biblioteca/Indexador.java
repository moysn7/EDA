...
public class Indexador {
    ...
    Map<String,ListaConPI<Indice>> indices;
     ...
     */ 
    private boolean cargarLibro(String fichLibro, String separadores)  {
        boolean res = true;
            try {
                ...
                System.out.println("Cargando ..." + fichLibro);
                int numLin = 0;
                ...
                while (libro.hasNext()) {
                    ...
                    for (int i = 0; i < words.length; i++) {
                        ...
                        if (esTermino(words[i])) {
                            t1 = System.nanoTime();
                            ListaConPI<Indice> palEnHash = indices.recuperar(words[i]);
                            if (palEnHash == null) {
                                ListaConPI<Indice> pal = new LEGListaConPI<Indice>();
                                pal.insertar(ind);
                                indices.insertar(words[i], pal);
                            } else {
                ...
                System.err.println("Error "+fichLibro+"/"+fichLibro + " no se encuentra");
                res = false;        
            }
            return res;
    }
    ...
    public ListaConPI<String> indiceDe(String pal) {
        ...
        ListaConPI<Indice> aux = new LEGListaConPI<Indice>();

        pal = pal.toLowerCase();
        aux = indices.recuperar(pal);
        ...
        if(aux != null){
            ...
            while(!aux.esFin()){
                lista.insertar(aux.recuperar().toString());
                aux.siguiente();
            }
        }
