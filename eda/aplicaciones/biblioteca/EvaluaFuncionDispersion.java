...
public class EvaluaFuncionDispersion {
    ...
    public final static String NOM_DIR = Indexador.DIR_LISTAS_LIBROS + File.separator + "res";
    public final static String NOM_FICH = NOM_DIR + File.separatorChar + "histo"; 
    public final static String EXT = ".txt";
    
    public static void main(String[] args) {
        int fdis = 0;
        TablaHash<Termino, Termino> tabla;
        try {
            ...
            for (fdis = 0; fdis < Termino.NOMFDIS.length; fdis++) {  
                ...
                tabla = construirTabla("lista10.txt", fdis);
				//COMPLETAR
				
                //2.- Mostrar por pantalla los valores de factor de carga y desviacion tipica
                System.out.println(Termino.NOMFDIS[fdis]);
                System.out.println("Factor de carga: "+tabla.factorCarga());
                System.out.println("Desviación típica: "+tabla.desviacionTipica());
                System.out.println("Factor de carga: "+tabla.factorCarga());
				//COMPLETAR               
	...
	boolean res = true;   	
        try {            
            Scanner libro = new Scanner(new File(nomLibro)); System.out.println("..." + libro);
            while (libro.hasNext()) {
